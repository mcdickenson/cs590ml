...
The authors present several experiments comparing the coalescent model to related methods such as Bayesian Hierarchical Clustering (BHC) and the Hierarchical Dirichlet Process (HDP). 
...
A disadvantage of the coalescent method is that it is less efficient to compute than BHC (O$(n^2)$ for a greedy coalescent implementation versus O$(n\log n)$ for BHC). If computational efficiency is crucial and information potentially shared across the tree is minimal, BHC would be preferable to the $n$-coalescent. 
